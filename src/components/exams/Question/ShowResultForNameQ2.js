...
const ShowResultForNameQ2 = React.forwardRef((props, ref) => {
  ...
  useEffectSkipFirst(() => {
    if (calscoreoftestJustForShowResultData) {
      ...
      if (calscoreoftestJustForShowResultData.voiceId) {
        console.log("voiceId", calscoreoftestJustForShowResultData.voiceId);

        DownloadVoiceFile(calscoreoftestJustForShowResultData.voiceId);
      }
      ...
      const catData = calscoreoftestJustForShowResultData.CritriaScores.map(
        (item) => {
          ...
          if (props.number === 116) {
            seriesList.push(item.userFrequencyScore);
            // seriesList.push(item.userPowerScore);
          } else {
    ...
    <div ref={ref} id="AnswerStyle1">
      ...
      </div>
      {
        <div className="pdfStyle">
          {!loading && pdfFile ? <AllPagesPDFViewer pdf={pdfFile} /> : null}
        </div>
      }
      {<div></div>}
    </div>
